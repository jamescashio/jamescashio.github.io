...
## 2024-07-10 - [DOM XSS] Eradicating innerHTML from dynamic DOM creation
**Vulnerability:** Found multiple instances of `element.innerHTML` being used to dynamically construct UI components (sound toggles, modals, alerts) in `cashio-us-v22.html`. While some inputs were ostensibly safe variables, this pattern inherently exposes the DOM to XSS and clobbering risks if user input flows reach these injection points.
**Learning:** `innerHTML` even for constructing static or semi-static string templates sets a poor, auditable precedent. Prior attempts to mitigate XSS by using `DOMParser` fall short if tags aren't explicitly filtered and reconstructed. Pure DOM API manipulation is the only strictly sound mitigation for constructing UI.
**Prevention:** Strictly enforce the use of safe DOM traversal and manipulation APIs (`document.createElement`, `document.createTextNode`, `textContent`, `appendChild`) over `innerHTML`. No exceptions, even for static assignments or "trusted" properties.
