...
## 2026-06-27 - DOM XSS via InnerHTML Assignment
**Vulnerability:** Directly assigning user-controlled or complex strings (e.g., terminal output formatting) to `innerHTML` creates DOM XSS vulnerabilities and DOM clobbering risks.
**Learning:** `DOMParser` alone does not neutralize malicious tags (like `<img onerror=...>`) during parsing. A custom node traversal mechanism (allowlisting safe elements like `SPAN`, `A`, `CODE` and reconstructing them via `document.createElement`) must be implemented if a dedicated sanitization library (like DOMPurify) is unavailable. When sanitizing URLs in `href` attributes, always strip whitespace and control characters (e.g., `[\x00-\x20\x7F]`) before regex matching to prevent whitespace bypass attacks on `javascript:` schemes.
**Prevention:** Avoid `innerHTML` entirely. Enforce safe DOM APIs (`document.createElement`, `textContent`, `appendChild`). If HTML string parsing is mandatory without third-party libraries, use `DOMParser` combined with strict allowlisting and careful attribute sanitization before appending to the DOM.

## 2025-05-27 - [Missing Authentication on Internal AI Bridge]
**Vulnerability:** The internal speculative decoding bridge (`scripts/zeusapollo_bridge.py`) exposed sensitive inference endpoints (`/v1/completions`) without any authentication or access control. While it resided on an internal network, this violated zero-trust principles and allowed unauthenticated actors to consume local AI inference resources, potentially leading to a Denial of Service (DoS) via resource exhaustion.
**Learning:** Even services deployed on internal, segmented networks (like the AI Swarm components) require strict authentication. A missing authentication layer on internal APIs often leads to unchecked resource consumption if an attacker gains a foothold.
**Prevention:** Apply a deny-by-default authentication middleware to all API endpoints, explicitly allowing only required public endpoints (like `/health` or documentation). Always enforce authentication (e.g., using API keys validated with `secrets.compare_digest()`) to verify caller identity before processing resource-intensive requests.
