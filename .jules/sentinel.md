...
## 2026-07-01 - [Missing Authentication in AI Bridge API]\n**Vulnerability:** The `/v1/completions` endpoint in `scripts/zeusapollo_bridge.py` lacked authentication checks, allowing unrestricted access to the speculative decoding bridge and potentially exposing internal inference resources to abuse.\n**Learning:** Internal microservices or secondary bridge scripts are often overlooked for security controls compared to primary gateways. All exposed endpoints, regardless of their role in the architecture, require robust authentication.\n**Prevention:** Implement a deny-by-default security middleware for all FastAPI applications that enforces API key validation using constant-time comparison (`secrets.compare_digest`) across all routes except explicitly defined public endpoints.
