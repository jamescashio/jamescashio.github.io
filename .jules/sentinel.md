...
## 2024-06-28 - [Missing Authentication in Python Bridge Endpoints]
**Vulnerability:** The `/v1/completions` endpoint in `scripts/zeusapollo_bridge.py` lacked API key authentication, exposing the speculative decoding bridge to unauthorized access over the network.
**Learning:** Secondary or specialized internal scripts (like caching or draft-generating bridges) often omit standard authentication layers present in the primary swarm routing scripts, creating security gaps that allow lateral access to inference infrastructure if the internal network is compromised.
**Prevention:** Always enforce a deny-by-default authentication middleware on all API servers regardless of their "internal" status. Verify API keys using constant-time comparison (`secrets.compare_digest()`), and structure authentication settings to load from environment variables with local config file fallbacks.
