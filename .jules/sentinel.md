...
## 2024-06-30 - [Missing Authentication on Backend Bridges]
**Vulnerability:** The internal API bridge `scripts/zeusapollo_bridge.py` lacked authentication for its endpoints (like `/v1/completions`), exposing inference capabilities to any local network caller without verification, while a similar swarm bridge implemented strict key-based checks.
**Learning:** Even internal middleware tools bridging local AI inference models require authentication and deny-by-default security configurations, otherwise they introduce an unauthenticated attack surface that can be exploited for resource exhaustion (DoS) or unauthorized model access.
**Prevention:** Always implement a deny-by-default middleware that explicitly enforces authentication (e.g., via `X-API-Key` or `Authorization` headers and `secrets.compare_digest`) on all incoming requests, explicitly allowing only known public endpoints like `/health` or documentation routes.
