...
## 2024-05-24 - [Bypass in Custom HTML Sanitization]
**Vulnerability:** A custom HTML sanitizer built using `DOMParser` to strip malicious attributes and tags allowed for a bypass by only matching exact strings like `javascript:`. Attackers could use control characters or spaces like `\x01javascript:` or ` javascript:` to bypass the filter and execute XSS payloads.
**Learning:** Browsers are highly tolerant of whitespaces and control characters in URL parsing for attributes like `href`. Simple string prefixes or `indexOf` checks are insufficient for validating URLs. Furthermore, omitting necessary tags like `<A>` or `<CODE>` in a strict allowlist breaks valid UI functionality.
**Prevention:** When sanitizing URLs to prevent XSS (e.g., in custom DOM traversals), always strip whitespace and control characters from the attribute value before checking for 'javascript:' schemes to prevent whitespace bypass attacks. Systematically verify and include all legitimate tags used by the application in the allowed tags array to avoid stripping valid content and causing UI regressions.
