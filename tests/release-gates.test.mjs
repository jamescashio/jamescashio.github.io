...
test("the preserved Odyssey document ships the complete V37 story with a usable V35 archive", async () => {
  for (const entry of ["odyssey.html"]) {
    ...
    assert.match(root.querySelector(".eh-release-mark").textContent, /V37.17.*PERSPECTIVE/);
...
test("V37 release manifests agree without redating the independent evidence archive", async () => {
  ...
  assert.equal(manifest.experienceVersion, "37.17.0");
...
test("Helios release identity, signature assets and compatibility receipts agree", async () => {
  ...
  assert.equal(release.experienceVersion, "38.6.0");
  ...
  assert.match(doc.body.textContent, /V38\.6 \/ HELIOS/);
