...
export function setupStudies({ scenes, motion, copy }) {
  ...
  function renderRoute(run, updateUrl = true) {
    ...
    box.dataset.run = String(run);
    box.dataset.hold = String(r.code === "HOLD");
    box.innerHTML = r.steps
      .map(
        (t, i) =>
          `<div class="step" data-step><span class="mono step-n">0${i + 1}</span><span class="dot step-dot"></span><span class="step-text">${t}</span></div>`,
      )
      .join("");
    const fg = $("#ringfg"),
      tx = $("#ringtxt");
