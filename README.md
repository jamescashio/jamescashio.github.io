...
**V39.2** is a code pass on Zenith. The front door's JavaScript is type checked: `tsconfig.helios.json` turns on `checkJs` for `src/helios`, the build runs it, and the modules carry the JSDoc types the check needs. The late `zenith.css` layer is folded into the stylesheets that own each component, `refinements.css` becomes `instruments.css` with its page rules joining `base.css`, and declarations a later rule always overrode are gone, so the entry stylesheet is 18,950 of 19,000 bytes gzip. The release codename leaves module and class names. E.V.E. answers `about` and `contact` in the page's own words and suggests the closest command after a typo.
