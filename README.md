...
**V39.2** makes the front door easier to maintain and its controls easier to use. `tsconfig.helios.json` type checks `src/helios` during every build. Each style rule now lives with its component, and the entry stylesheet remains within its 19,000 byte gzip budget. Focus and privacy feedback stay visible, Motion keeps a stable accessible name, and the glossary opens on request. E.V.E. answers `about` and `contact`, suggests a command after a typo, and lets keyboard visitors leave after completing a command. The arrival film waits until it can play, and the saved mission card uses the original signature without a dark rectangle. Reading editions are linked from the room directory and each room, the experiment chooser is one action away, and 3G connections use the still arrival artwork. Original assets, public evidence and observation dates are preserved.
